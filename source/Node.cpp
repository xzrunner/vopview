...
void Node::InitPins(const std::string& name)
{
	...
	assert(var_imports.is_valid()
		&& var_imports.is_type<std::vector<hdiop::Node<vop::VariableType>::Port>>());
	auto& imports = var_imports.get_value<std::vector<hdiop::Node<vop::VariableType>::Port>>();
	...
	assert(var_exports.is_valid()
		&& var_exports.is_type<std::vector<hdiop::Node<vop::VariableType>::Port>>());
	auto& exports = var_exports.get_value<std::vector<hdiop::Node<vop::VariableType>::Port>>();

	auto rg2grp = [](std::vector<PinDesc>& dst, const std::vector<hdiop::Node<vop::VariableType>::Port>& src)
	{
